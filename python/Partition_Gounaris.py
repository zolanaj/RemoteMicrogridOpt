...
#                if tech.GetType() != "PVArray"]))  
    ...
    p.variables.add(obj=scipy.array(fuel_costs[start_period-1:end_period]),
                    ...
                    names=F_tilde_names )
    #B_soc_bkt -- State of charge of battery b, twin k at end of time period t
    ...
    #constraint 16a1 -- Generator Max Power
    for gen in generators:
        for k in range(tech_maxes[gen.GetName()]):
            p.linear_constraints.add(lin_expr=[
    ...
    #constraint 16d -- Break generator use symmetry                                                                                                                                                      
    for gen in generators:
    ...
    #constraint 16e -- Break generator power out symmetry
    for gen in generators:
        for k in range(tech_maxes[gen.GetName()]-1):
            p.linear_constraints.add(lin_expr=[
                    cplex.SparsePair([
                    ...
                    senses=['G' for t in range(num_periods)], 
                rhs=scipy.zeros(num_periods,dtype=float), 
                names=['Gen_Op_16e_'+gen.GetName()+".K"
                +str(k+2)+".TT"+str(t+start_period)
                for t in range(num_periods)])
    ...
    #constraint 17b -- upper bound on PV system purchases
    for pv in pv_arrays:
        p.linear_constraints.add(lin_expr=[
                cplex.SparsePair(["X_"+pv.GetName()],
                    [1.0])],
                senses=['L'],
                rhs=[float(scalars["solar_max"])], 
        ...
        for bat in batteries:
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    senses=sense, 
                    rhs=[0], 
                    names=['Storage_inv_end_UB_19c2'+ending+bat.GetName()+".K"+str(k)]) 
    ...
    #constraint 25a -- lambda assignment based on previous SOC, current dir.
    for bat in batteries:
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_23a_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period,
                                    end_period+1)])
    #constraint 25b -- lambda assignment based on previous SOC, current dir.
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*bat.GetIUPlus(),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_25b_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period,
                                    end_period+1)])
    #constraint 25c -- lambda assignment based on previous SOC, current dir.
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*bat.GetMinCurrent(),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_25c_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period,
                                    end_period+1)])
    #constraint 25d -- lambda assignment based on previous SOC, current dir.
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*bat.GetIUMinus(),
    ...
    #constraint 25e -- one lambda per BKT - reconcile plus.
    ...
    #constraint 25f -- one lambda per BKT - reconcile minus.
    for bat in batteries:
        for k in range(1,tech_maxes[bat.GetName()]+1):
             p.linear_constraints.add(
...
#    if output:
#        print (sol["W"])
#    print (flatten(lambda_minus_names))
    del p
    return sol     
    

def OutputUBSolutions(ub_sols,output_dir="./../sol_files/",scenario_name = "ll12"):
...
def OutputAllSols(sols, output_dir = "./../sol_files/", scenario_name = "ll12"):
...
def SolveScenarioLBPartition(params):  
    ...
    sol = SolvePartition(inputs,params[4],
        ...
        mipgap=0.01,reslim=params[10],IFS=params[8],mincap=params[9],
...
if __name__ == "__main__":
    ...
    #Solve day-long problem as a test
    lb,obj = RunLinearLBDay(scenario, lambs=[0.0],num_days=1, probSize=24,
        ...
        reslim=60, socParts=[0.0,1.0],
        output=True)
    elapsed = time.time()-clock
    print (scenario, elapsed)
