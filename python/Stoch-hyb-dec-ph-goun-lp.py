...
import Stoch_Partition_Current_lp_goun
...
#    obj = sum([sol["obj"] for sol in sols])
    ...
    obj_ub,sols_ub,frac_over = Stoch_Partition_Current_lp_goun.RunParallelUBDay(scenario,
            num_days=num_days,
            probSize=probSize,
...
def RunPH(scenario,num_days=365,probSize=24,itermax=40,tol=0.05,n=3,ubCheck=5,
    ...
    sol_ub = None
#    if nonuniform_part:
#        lb,sol_lb,frac_over = Stoch_Partition_Current_lp_goun.RunParallelLBDay(scenario,lambs=lambs,
#                design_mults=design_mults,num_days=num_days,probSize=probSize,
...
#        bat_parts_minus = GetCurrentParts(sol_lb,"I_minus",numParts) 
    ...
    #for sol in sol_lb:
    #    print sol["W"],sol["X"]
    #Stoch_Partition_Current_lp_goun.outputSolsLambs(scenario,sol_lb,0,False)
    incumbent=1
    ...
    while gap > tol and k <= itermax and elapsed < timeLimit:
        ...
        #del lb
        #del sol_lb 
        lb,sol_lb,frac_over = Stoch_Partition_Current_lp_goun.RunParallelLBDay(scenario,
...
#        Stoch_Partition_Current_lp_goun.outputSolsLambs(scenario,sol_lb,k,False)
        if nonuniform_part:
            bat_parts_plus = GetCurrentParts(sol_lb,"I_plus",numParts) 
            bat_parts_minus = GetCurrentParts(sol_lb,"I_minus",numParts) 
...
#            sub = str(sol_ub)
            ...
            for sol in sol_lb: 
                ...
                if str(sol["W"])+str(sol["X"]) not in ub_sols:
                    ...
                    #Stoch_Partition_Current_lp_goun.OutputDesign([sol],output_dir = scenario+"/", scenario_name = scenario)
                    Stoch_Partition_Current_lp_goun.OutputAllSols([sol],output_dir = IFS_dir, scenario_name = scenario)
                    objU,solU,frac_over = Stoch_Partition_Current_lp_goun.RunParallelUBDay(scenario,num_days=num_days,probSize=probSize,batPartsPlus=bat_parts_plus,
                    ...
                    if objU < obj:
                        if MINLP:
                            Stoch_Partition_Current_lp_goun.OutputAllSols(solU,IFS_dir,scenario)
...
def FindBestSOC(sol_ub, obj, gap, incumbent, scenario,num_days=365,
    ...
    soc_mid = 0.5
    Stoch_Partition_Current_lp_goun.OutputAllSols([sol_ub[0]],output_dir = scenario+"/", scenario_name = scenario)
    o_mid,sol_mid,frac_over = Stoch_Partition_Current_lp_goun.RunParallelUBDay(scenario,
    ...
    while dist > tol:
        soc_hi = soc_mid + dist
        soc_lo = soc_mid - dist
        o_hi,s_hi,frac_over = Stoch_Partition_Current_lp_goun.RunParallelUBDay(scenario,
            num_days=num_days,probSize=probSize,batPartsPlus=batPartsPlus,
            batPartsMinus=batPartsMinus, boundary_soc=soc_hi)
        o_lo,s_lo,frac_over = Stoch_Partition_Current_lp_goun.RunParallelUBDay(scenario,
            num_days=num_days,probSize=probSize,batPartsPlus=batPartsPlus,
        ...
        dist /= 2
    if MINLP:
        Stoch_Partition_Current_lp_goun.OutputAllSols(sol_mid,output_dir = scenario+"/", scenario_name = scenario)
...
if __name__ == "__main__":
    ...
    #print design_mults
    outfile = open("results_gounlp_p"+str(numParts)+"_"+scenario+".csv","w") 
    start = time.time()
