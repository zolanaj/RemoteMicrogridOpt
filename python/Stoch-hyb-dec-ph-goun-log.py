...
import Stoch_Partition_Current_ph_goun
...
#    obj = sum([sol["obj"] for sol in sols])
    ...
    obj_ub,sols_ub,frac_over = Stoch_Partition_Current_ph_goun.RunParallelUBDay(scenario,
            num_days=num_days,
            probSize=probSize,
...
def RunPH(scenario,num_days=365,probSize=24,itermax=40,tol=0.05,n=3,ubCheck=5,
        ...
        lbFind = 5,numParts=5, MINLP=True, nonuniform_part=False, findgen=True,
        timeLimit = 3600, solsToCheck=10):
    ...
    numParts -- number of subregions for current variable
    ...
    """
    import os
    iterfile = open("iter-scen"+scenario+"-P"+str(numParts)+".csv",'w')
    ...
    bat_parts_plus = scipy.arange(numParts+1)*1.0/numParts
    ...
    if findgen:
        ...
        print ("mincap:",mincap)
        if nonuniform_part:
            bat_parts_plus = GetCurrentParts(sols,"I_plus",numParts) 
            bat_parts_minus = GetCurrentParts(sols,"I_minus",numParts) 
    else:
        mincap = 0
    #obtain upper bound.
    obj = scipy.infty
    ...
    sol_ub = None
#    if nonuniform_part:
#        lb,sol_lb,frac_over = Stoch_Partition_Current_ph_goun.RunParallelLBDay(scenario,lambs=lambs,
#                design_mults=design_mults,num_days=num_days,probSize=probSize,
...
#        bat_parts_minus = GetCurrentParts(sol_lb,"I_minus",numParts) 
    ...
    #for sol in sol_lb:
    #    print sol["W"],sol["X"]
    #Stoch_Partition_Current_ph_goun.outputSolsLambs(scenario,sol_lb,0,False)
    incumbent=1
    ...
    while gap > tol and k <= itermax and elapsed < timeLimit:
        ...
        #del lb
        #del sol_lb 
        lb,sol_lb,frac_over = Stoch_Partition_Current_ph_goun.RunParallelLBDay(scenario,
        ...
        return lb,0,0,0,frac_over
#        Stoch_Partition_Current_ph_goun.outputSolsLambs(scenario,sol_lb,k,False)
        if nonuniform_part:
            bat_parts_plus = GetCurrentParts(sol_lb,"I_plus",numParts) 
            bat_parts_minus = GetCurrentParts(sol_lb,"I_minus",numParts) 
        ...
        if lb > incumbent:
            ...
            elapsed = time.time() - clock
            iterfile = open("iter-scen"+scenario+"-P"+str(numParts)+".csv",'a')
        ...
        if k % ubCheck == 0:
            ...
            for sol in sol_lb: 
                ...
                if str(sol["W"])+str(sol["X"]) not in ub_sols:
                    ...
                    #Stoch_Partition_Current_ph_goun.OutputDesign([sol],output_dir = scenario+"/", scenario_name = scenario)
                    Stoch_Partition_Current_ph_goun.OutputAllSols([sol],output_dir = IFS_dir, scenario_name = scenario)
                    objU,solU,frac_over = Stoch_Partition_Current_ph_goun.RunParallelUBDay(scenario,num_days=num_days,probSize=probSize,batPartsPlus=bat_parts_plus,
                    ...
                    if objU < obj:
                        if MINLP:
                            Stoch_Partition_Current_ph_goun.OutputAllSols(solU,IFS_dir,scenario)
...
def FindBestSOC(sol_ub, obj, gap, incumbent, scenario,num_days=365,
    ...
    soc_mid = 0.5
    Stoch_Partition_Current_ph_goun.OutputAllSols([sol_ub[0]],output_dir = scenario+"/", scenario_name = scenario)
    o_mid,sol_mid,frac_over = Stoch_Partition_Current_ph_goun.RunParallelUBDay(scenario,
    ...
    while dist > tol:
        soc_hi = soc_mid + dist
        soc_lo = soc_mid - dist
        o_hi,s_hi,frac_over = Stoch_Partition_Current_ph_goun.RunParallelUBDay(scenario,
            num_days=num_days,probSize=probSize,batPartsPlus=batPartsPlus,
            batPartsMinus=batPartsMinus, boundary_soc=soc_hi)
        o_lo,s_lo,frac_over = Stoch_Partition_Current_ph_goun.RunParallelUBDay(scenario,
            num_days=num_days,probSize=probSize,batPartsPlus=batPartsPlus,
        ...
        dist /= 2
    if MINLP:
        Stoch_Partition_Current_ph_goun.OutputAllSols(sol_mid,output_dir = scenario+"/", scenario_name = scenario)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2: numParts = int(sys.argv[2])
    else: numParts = 4
    ...
    #print design_mults
    outfile = open("results_gapsgoun1_p"+str(numParts)+"_"+scenario+".csv","w") 
    start = time.time()
    lb,obj,gap,iters,frac_over = RunPH(scenario,num_days=365,probSize=24,
            itermax=100,tol=0.95,n=50,ubCheck=3,
            tech_filename = tech_filename,
            max_filename = max_filename,
            pv_spec_filename = pv_spec_filename,
            lbFind=1, numParts=numParts, MINLP=False,nonuniform_part=False,
            findgen=True, timeLimit=7200, solsToCheck=20
            )
    #
    elapsed = (time.time() - start) #measure time elapsed in minutes
    print("time elapsed to answer:",elapsed)
    outfile.write("Scenario,LB,UB,gap,iters,time,fracOverLim\n")
    outfile.write(scenario+"goun,"+str(lb)+","+str(obj)+","+str(gap)+","+str(iters)+","+str(elapsed)+","+str(frac_over)+","+"\n")
