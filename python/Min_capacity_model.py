...
def SolvePartition(inputs, scenario, start_period, end_period, 
        soc_mult=0.0, upper_bound = False, mipgap=0.05, 
        ...
        output=False, amortize_on_demand = True, min_PV=0,procs=1):
    """creates a full partition of the model.
    start_period -- starting period for the model
    end_period -- ending period for the model
    soc_mult -- price of SOC constraint violation (start = end = reset point)
    ...
    def B_soc_var(bat,k,t): return "B_soc_" + bat.GetName() + ".K" + str(k) + ".TT" + str(t) 
    ...
    V_soc_names = [[["V_soc_" + tech.GetName() + ".K" + str(i+1) 
            ...
            for tech in technologies 
                    if tech.GetType() == "Battery"] 
            for t in range(start_period, end_period+1)]
    Y_minus_names = [[["Y_minus_" + tech.GetName() + ".K" + str(i+1) 
             + ".TT" + str(t) for i in range(tech_maxes[tech.GetName()])] 
            for tech in technologies 
                    if tech.GetType() == "Battery"] 
            for t in range(start_period, end_period+1)]
    Y_plus_names = [[["Y_plus_" + tech.GetName() + ".K" + str(i+1) 
             + ".TT" + str(t) for i in range(tech_maxes[tech.GetName()])] 
            for tech in technologies 
                    if tech.GetType() == "Battery"] 
            for t in range(start_period, end_period+1)]
    ...
    purchase_costs = ( scipy.array( flatten( [[tech.GetMaxPower() 
            for i in range(tech_maxes[tech.GetName()])] 
            for tech in technologies 
            if tech.GetType() == "DieselGenerator"]) + 
            flatten( [[0.0 
            for i in range(tech_maxes[tech.GetName()])] 
            for tech in technologies 
            if tech.GetType() == "Battery"]) ) ) + design_mults[0]
    solar_costs = ( scipy.array([
            0.0 for tech in technologies 
    ...
    #L_bk -- Lifecycles
    p.variables.add(obj=scipy.zeros_like(lifecycle_costs,dtype=float),
                    lb=scipy.zeros_like(lifecycle_costs,dtype=float),
                    ub=[cplex.infinity]*len(lifecycle_costs),  
                    types=[p.variables.type.continuous]*len(lifecycle_costs), 
                    names= flatten(L_names) )
    #X_s -- Solar Purchase Costs
    p.variables.add(obj=scipy.zeros_like(solar_costs,dtype=float),
    ...
    p.variables.add(obj=0.0*scipy.array(fuel_costs[start_period-1:end_period]),
    ...
    p.variables.add(obj=scipy.zeros_like(Ghat_flat,dtype=float),
    ...
    #if start_period != 1:
    p.variables.add(obj=soc_mult*lambda_multipliers,
                    lb=[0.0]*num_soc,
                    ub=[1.0]*num_soc,
                    types=[p.variables.type.continuous]*num_soc,
                    names=flatten(soc_start_names) )
    #if end_period != 8760:
    p.variables.add(obj=[0.0]*num_soc,
                    lb=[0.0]*num_soc,
                    ub=[1.0]*num_soc,
                    types=[p.variables.type.continuous]*num_soc,
    ...
    for bat in batteries:
        ...
        for k in range(tech_maxes[bat.GetName()]):
            p.linear_constraints.add(lin_expr=[
                    ...
                    senses=['E' for t in range(1,num_periods)],
        ...
        for k in range(tech_maxes[bat.GetName()]):
            p.linear_constraints.add(lin_expr=[
                    cplex.SparsePair([I_minus_var(bat,k+1,t),
                                B_soc_var(bat,k+1,t-1)]
                            ,val) for t in range(start_period+1,end_period+1)],
                    senses=['L' for t in range(1,num_periods)],
                    rhs=scipy.zeros(num_periods-1), 
    ...
    def i_u_part_minus(bat,n):
        return bat.GetMinCurrent() + batPartsMinus[n-1]*(bat.GetIUMinus()-bat.GetMinCurrent())
    def i_u_part_plus(bat,n):
        return bat.GetMinCurrent() + batPartsPlus[n-1]*(bat.GetIUPlus()-bat.GetMinCurrent())
    
    #constraint 23a -- lambda assignment based on previous SOC, current dir.
    for bat in batteries:
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_23a_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period,
                                    end_period+1)])
    #constraint 23b -- lambda assignment based on previous SOC, current dir.
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*bat.GetIUPlus(),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_23b_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period,
                                    end_period+1)])
    #constraint 23c -- lambda assignment based on previous SOC, current dir.
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*bat.GetMinCurrent(),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_23c_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period,
                                    end_period+1)])
    #constraint 23d -- lambda assignment based on previous SOC, current dir.
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*bat.GetIUMinus(),
    ...
    #constraint 23e -- one lambda per BKT - reconcile plus.
    val = [1.0 for i in range(2,numBatParts+1)]+[-1.0]
    for bat in batteries:
        for k in range(1,tech_maxes[bat.GetName()]+1):
             p.linear_constraints.add(
                    lin_expr=[cplex.SparsePair([lambda_plus_var(bat,k,n,t) 
                        for n in range(2,numBatParts+1)]+[B_plus_var(bat,k,t)],
                            val) for t in range(start_period,
                                    end_period+1)],
                    senses=['E' for t in range(num_periods)], 
                    rhs=scipy.zeros(num_periods), 
    ...
    #constraint 23f -- one lambda per BKT - reconcile minus.
    for bat in batteries:
        for k in range(1,tech_maxes[bat.GetName()]+1):
             p.linear_constraints.add(
                    lin_expr=[cplex.SparsePair([lambda_minus_var(bat,k,n,t) 
                        for n in range(2,numBatParts+1)]+[B_minus_var(bat,k,t)],
                            val) for t in range(start_period,
                                    end_period+1)],
                    senses=['E' for t in range(num_periods)], 
                    rhs=scipy.zeros(num_periods), 
                    names=['Linear_23f_'+bat.GetName()+".K"+str(k)+
                        ".TT"+str(t) for t in range(start_period,
                                    end_period+1)]) 
    #                                
    ##constraint 23g -- Z_plus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                ...
                -1.0*(bat.GetMaxSOC()-bat.GetMinSOC())*
                    (bat.GetIUPlus()-i_u_part_plus(bat,n)),
                1.0*(bat.GetMaxSOC()-bat.GetMinSOC())*
                    (bat.GetIUPlus()-i_u_part_plus(bat,n)) + 
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    lin_expr=[cplex.SparsePair([
                            ...
                            lambda_plus_var(bat,k,n,t),
                            W_var(bat,k)],
                            val) for t in range(start_period+1,
                                    end_period+1)],
                    senses=['G' for t in range(1,num_periods)], 
                    rhs=scipy.zeros(num_periods-1), 
                    names=['Linear_23g_'+bat.GetName()+".K"+str(k)+
                        ".N"+str(n)+
                        ".TT"+str(t) for t in range(start_period+1,
                                    end_period+1)])
    ##constraint 23h-- Z_plus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                ...
                -1.0*bat.GetMinSOC(),
                -1.0*(bat.GetMaxSOC()-bat.GetMinSOC())*
                    (i_u_part_plus(bat,n-1)-bat.GetMinCurrent()),
                1.0*(bat.GetMaxSOC()-bat.GetMinSOC())*
                    (i_u_part_plus(bat,n-1)-bat.GetMinCurrent())+
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_23h_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period+1,
                                    end_period+1)])  
    ##constraint 23i -- Z_plus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*i_u_part_plus(bat,n-1),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_23i_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period+1,
                                    end_period+1)])
    ##constraint 23j -- Z_plus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*i_u_part_plus(bat,n),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_23j_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period+1,
                                    end_period+1)])   
    ##constraint 23k -- Z_minus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                ...
                1.0*bat.GetMaxSOC(),
                -1.0*(bat.GetMaxSOC()-bat.GetMinSOC())*
                    (bat.GetIUMinus()-i_u_part_minus(bat,n)),
                1.0*(bat.GetMaxSOC()-bat.GetMinSOC())*
                    (bat.GetIUMinus()-i_u_part_minus(bat,n))+
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_23k_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period+1,
                                    end_period+1)])
    ##constraint 23l-- Z_minus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                ...
                -1.0*bat.GetMinSOC(),
                -1.0*(bat.GetMaxSOC()-bat.GetMinSOC())*
                        (i_u_part_minus(bat,n-1)-bat.GetMinCurrent()),
                1.0*(bat.GetMaxSOC()-bat.GetMinSOC())*
                        (i_u_part_minus(bat,n-1)-bat.GetMinCurrent())+
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_23l_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period+1,
                                    end_period+1)])  
    ##constraint 23m -- Z_minus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*i_u_part_minus(bat,n-1),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_23m_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period+1,
                                    end_period+1)])
    ##constraint 23n-- Z_minus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*i_u_part_minus(bat,n),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    names=['Linear_23n_'+bat.GetName()+".K"+str(k)+
                        ...
                        ".TT"+str(t) for t in range(start_period+1,
                                    end_period+1)])   
    #constraint 23o -- Z_minus <= I_minus
    for bat in batteries:
        val = [1.0,-1.0]  
        for k in range(1,tech_maxes[bat.GetName()]+1):
            p.linear_constraints.add(
    ...
    ##constraint Init_Cond_23g -- Z_plus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*i_u_part_plus(bat,n),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    lin_expr=[cplex.SparsePair([
                            Z_plus_var(bat,k,start_period),
                            soc_start_var(bat,k),
                            I_plus_var(bat,k,start_period),
                            lambda_plus_var(bat,k,n,start_period),
                            W_var(bat,k)],
                            val)],
                    senses=['G'], 
                     rhs=[0], 
                    names=['Init_Cond_23g_'+bat.GetName()+".K"+str(k)+
                        ".N"+str(n)])
    ##constraint Init_Cond_23h-- Z_plus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*i_u_part_plus(bat,n-1),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    senses=['G'], 
                    rhs=[0], 
                    names=['Init_Cond_23h_'+bat.GetName()+".K"+str(k)+
                        ".N"+str(n)])  
    ##constraint Init_Cond_23i -- Z_plus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*i_u_part_plus(bat,n-1),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    senses=['L'], 
                    rhs=[0],  
                    names=['Init_Cond_23i_'+bat.GetName()+".K"+str(k)+
                        ".N"+str(n)])
    ##constraint Init_Cond_23j-- Z_plus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*i_u_part_plus(bat,n),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    senses=['L'], 
                    rhs=[0], 
                    names=['Init_Cond_23j_'+bat.GetName()+".K"+str(k)+
                        ".N"+str(n)])   
    #constraint Init_Cond_23k -- Z_minus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*i_u_part_minus(bat,n),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    lin_expr=[cplex.SparsePair([
                            Z_minus_var(bat,k,start_period),
                            soc_start_var(bat,k),
                            I_minus_var(bat,k,start_period),
                            lambda_minus_var(bat,k,n,start_period),
                            W_var(bat,k)],
                            val)],
                    senses=['G'], 
                    rhs=[0], 
                    names=['Init_Cond_23k_'+bat.GetName()+".K"+str(k)+
                        ".N"+str(n)])
    #constraint Init_Cond_23l-- Z_minus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*i_u_part_minus(bat,n-1),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    senses=['G'], 
                    rhs=[0],
                    names=['Init_Cond_23l_'+bat.GetName()+".K"+str(k)+
                        ".N"+str(n)])  
    #constraint Init_Cond_23m -- Z_minus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*i_u_part_minus(bat,n-1),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    senses=['L'], 
                    rhs=[0],
                    names=['Init_Cond_23m_'+bat.GetName()+".K"+str(k)+
                        ".N"+str(n)])
    #constraint Init_Cond_23n-- Z_minus linearization
    for bat in batteries:
        for n in range(2,numBatParts+1):
            val = [1.0,
                -1.0*i_u_part_minus(bat,n),
            ...
            for k in range(1,tech_maxes[bat.GetName()]+1):
                p.linear_constraints.add(
                    ...
                    senses=['L'], 
                    rhs=[0], 
                    names=['Init_Cond_23n_'+bat.GetName()+".K"+str(k)+
                        ".N"+str(n)])  
    ...
    for bat in batteries:
        for k in range(1,tech_maxes[bat.GetName()]+1):
            p.linear_constraints.add(
                ...
                senses=sense, 
                rhs=[0], 
                names=['RESET_SOC_start_'+ending+bat.GetName()+".K"+str(k)])  
    ...
    try: 
        ...
        sol["lamb_start"] = soc_mult
        sol["lamb_end"] = soc_mult
...
def SolveScenarioLBPartition(params):  
    ...
    sol = SolvePartition(inputs,params[4],
        ...
        mipgap=0.01,reslim=120,IFS=params[8])
    #print "LB", sol["lb"]
    return sol
...
def RunParallelLBDay(scenario, lambs=[0.0]*365, 
            procs=multiprocessing.cpu_count(),num_days=365, probSize=24,
             ...
             design_mults=[(0,0)]*365,amortize_on_demand = True, IFS=None):
    if IFS == None: IFS = [None]*num_days
    params_list = [(i*probSize+1,(i+1)*probSize,lambs[i],0.0,scenario,
            batPartsPlus,batPartsMinus,design_mults[i],IFS[i]) for i in range(num_days)]
...
def RunParallelUBDay(scenario, lambs=[0.0]*365,  procs=multiprocessing.cpu_count(), 
        ...
        boundary_soc=0.5):
    params_list = [(i*probSize+1,(i+1)*probSize,lambs[i],0.0,scenario,
            batPartsPlus,batPartsMinus,boundary_soc) for i in range(num_days)]
    pool = multiprocessing.Pool(processes=procs)
    #t = time.time()
...
def RunLinearLBDay(scenario, lambs=[0.0]*365,num_days=365, probSize=24,
             batPartsPlus=[0,0.25,0.5,0.75,1.0],batPartsMinus=[0,0.25,0.5,0.75,1.0],
             design_mults=[(0,0)]*365,amortize_on_demand = True, IFS=None):
    if IFS == None: IFS = [None]*num_days
    params_list = [(i*probSize+1,(i+1)*probSize,lambs[i],0.0,scenario,
...
def outputSolsLambs(scenario,sols,k,ub):
    ...
    lbfile = open("Design"+scenario+"lb10.csv",writetype)
    lambfile = open("Design"+scenario+"lambs10.csv",writetype)
    stfile = open("Design"+scenario+"starts10.csv",writetype)
    endfile = open("Design"+scenario+"ends10.csv",writetype)
    gapfile = open("Design"+scenario+"gaps10.csv",writetype)
    if ub:
        lambfile.write('iter')
        stfile.write('iter')
        endfile.write('iter')
...
def RunWholeProblem(scenario):
    timeclock = time.time()
    params_list = [(1,8760,0.0,0.0,scenario,[0.0,0.25,0.5,0.75,1.0])]
    obj=0.0
    append=False
    for param_tuple in params_list:
        print param_tuple
